...
namespace Poderosa.Communication
{
	...
	public abstract class TerminalConnection {
		...
		public void ResetLog(LogType t, string path, bool append, string controlName) { //edited by xavier: added controlname
			_logType = t;
			_logPath = path;
            ...
            _loggerT = new InternalLoggerT(_loggerT, this, controlName); //edited by xavier: added controlname
		...
		private class InternalLoggerT : ITerminalTextLogger {
            ...
            private string _controlName;//edited by xavier: added controlname
			public InternalLoggerT(ITerminalTextLogger l, TerminalConnection p, string controlName) { //edited by xavier: added controlname
				_parent = p;
				_logger = l;
            ...
            private NLog.Logger log = NLog.LogManager.GetCurrentClassLogger();//edited by xavier
			public void WriteLine(GLine line) {
                if (!_parent.LogSuspended)
                {
                    _logger.WriteLine(line);
                    string s = new string(line.Text).Trim('\0');//edited by xavier
                    //Trace.Write(s); Trace.Write("\r");//edited by xavier
                    log.Info(_controlName + ": " + s); //log.Info("\r");//edited by xavier
                }
			}
